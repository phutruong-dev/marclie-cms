...
| **Neon DEV db** | project `marclie-cms-dev` · id `floral-fire-25613544` · branch `main` · region `aws-eu-central-1` — **local dev only** (connection string in `.env`; media on local disk, no Blob). Never point prod here, never point local dev at `marclie-cms-prod`. |
